...
CHANGELOG = """
**v2.0** (2024-12-19)
- ✅ Added FMP API integration for reliable fundamental data
- ✅ Optimized to 2 API calls per stock
- ✅ Hybrid mode: FMP primary, Yahoo Finance fallback
- ✅ Full YoY data for all 4 quarters

**v1.1** (2024-12-18)
- Changed to YoY (Year-over-Year) growth calculations
- Added quarter date labels
- Improved missing data handling

**v1.0** (2024-12-18)
- Initial release with Yahoo Finance only
"""
...
if 'fmp_api_key' not in st.session_state:
    st.session_state.fmp_api_key = saved_inputs.get('fmp_api_key', '')

# ==================== FMP API FUNCTIONS (v2.0) ====================
...
def fetch_fmp_data(ticker, api_key):
    ...
    2. Key Metrics (quarterly)
    
    Returns: (income_data, metrics_data, error)
    """
    if not api_key or api_key.strip() == '' or api_key == 'demo':
        return None, None, "Please provide a valid FMP API key"
    
    try:
        # Call 1: Income Statement (quarterly) - Last 12 quarters
        ...
        if not income_data or len(income_data) == 0:
            return None, None, "No income data available from FMP"
        
        # Call 2: Key Metrics (quarterly) - For ROE
        ...
        return income_data, metrics_data, None
        
    except Exception as e:
        return None, None, f"Error fetching FMP data: {str(e)}"

def calculate_yoy_growth_fmp(income_data, metric_name='revenue'):
    """Calculate YoY growth for last 4 quarters using FMP data"""
    if not income_data or len(income_data) < 8:
        return [None, None, None, None]
    
    growth_rates = []
    for i in range(4):
        current = income_data[i].get(metric_name, 0)
        year_ago = income_data[i + 4].get(metric_name, 0)
        
        if year_ago != 0 and current is not None and year_ago is not None:
            yoy_growth = ((current - year_ago) / abs(year_ago)) * 100
...
def calculate_fmp_fundamentals(income_data, metrics_data):
    """Calculate all 5 fundamental indicators from FMP data"""
    scores = {
        'sales_growth': 0,
        'profit_margin': 0,
        'earnings': 0,
    ...
    # Extract basic data
    details['sales'] = [q.get('revenue', 0) for q in income_data[:4]]
    details['earnings'] = [q.get('netIncome', 0) for q in income_data[:4]]
    
    # 1. Sales Growth Acceleration (YoY)
    sales_growth = calculate_yoy_growth_fmp(income_data, 'revenue')
    ...
    margin_values = []
    for i in range(min(4, len(income_data))):
        revenue = income_data[i].get('revenue', 0)
        ebitda = income_data[i].get('ebitda', 0) or income_data[i].get('operatingIncome', 0)
    ...
    earnings_growth = calculate_yoy_growth_fmp(income_data, 'netIncome')
    ...
    # 4. Rule of 40 (latest quarter)
    if len(income_data) >= 5:  # Need current + 4 quarters back for YoY
        latest_revenue = income_data[0].get('revenue', 0)
        year_ago_revenue = income_data[4].get('revenue', 0)
        
        if year_ago_revenue != 0:
            revenue_growth = ((latest_revenue - year_ago_revenue) / abs(year_ago_revenue)) * 100
            details['latest_revenue_growth'] = revenue_growth
            
            latest_ebitda = income_data[0].get('ebitda', 0) or income_data[0].get('operatingIncome', 0)
            if latest_revenue != 0 and latest_ebitda is not None:
                profit_margin = (latest_ebitda / latest_revenue) * 100
    ...
    # 5. ROE from metrics data
    if metrics_data and len(metrics_data) > 0:
        roe_quarters = []
        for i in range(min(4, len(metrics_data))):
            roe = metrics_data[i].get('roe') or metrics_data[i].get('returnOnEquity')
            if roe is not None:
                roe_pct = roe * 100 if abs(roe) < 1 else roe
                roe_quarters.append(roe_pct)
            else:
...
with st.sidebar:
    ...
    st.caption("[Get free key](https://financialmodelingprep.com/) (250 calls/day)")
    
    fmp_key = st.text_input(
        "FMP API Key",
        value=st.session_state.fmp_api_key,
        type="password",
        help="Free tier: 250 calls/day = 125 stock analyses. Leave empty to use Yahoo Finance (limited data)"
    )
    ...
    if fmp_key != st.session_state.fmp_api_key:
        ...
        saved_inputs = load_user_inputs()
        saved_inputs['fmp_api_key'] = fmp_key
        save_user_inputs(saved_inputs)
    
    # Show data source status
    if fmp_key and fmp_key.strip() != '':
        st.success("✅ Using FMP API (reliable data)")
        st.caption("2 API calls per stock analysis")
    else:
...
if analyze_button or ticker:
    ...
    with st.spinner(f"Fetching data for {ticker}..."):
        ...
        # Try FMP first if API key is provided
        use_fmp = False
        if fmp_key and fmp_key.strip() != '':
            st.info("📡 Fetching from FMP API...")
            fmp_income, fmp_metrics, fmp_error = fetch_fmp_data(ticker, fmp_key)
            
            if not fmp_error and fmp_income:
                use_fmp = True
                st.success("✅ Using FMP data")
            else:
                st.warning(f"⚠️ FMP failed: {fmp_error}. Falling back to Yahoo Finance...")
                use_fmp = False
    ...
    with tab2:
        ...
        if use_fmp:
            st.info("📊 Using FMP API data (full YoY for all quarters)")
            fund_scores, fund_details = calculate_fmp_fundamentals(fmp_income, fmp_metrics)
            fund_error = None
        else:
            if fund_error:
                st.error(fund_error)
